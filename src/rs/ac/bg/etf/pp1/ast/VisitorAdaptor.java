// generated with ast extension for cup
// version 0.8
// 25/6/2020 14:34:23


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(Void Void) { }
    public void visit(ExprNeg ExprNeg) { }
    public void visit(DesignatorLbrack DesignatorLbrack) { }
    public void visit(StatementList StatementList) { }
    public void visit(ConstDeclSpec ConstDeclSpec) { }
    public void visit(Factor Factor) { }
    public void visit(IncDec IncDec) { }
    public void visit(VarSingleSpec VarSingleSpec) { }
    public void visit(Designator Designator) { }
    public void visit(Term Term) { }
    public void visit(WAss WAss) { }
    public void visit(VarDeclSpec VarDeclSpec) { }
    public void visit(MethType MethType) { }
    public void visit(ActualParamList ActualParamList) { }
    public void visit(VarDeclList VarDeclList) { }
    public void visit(LParenCondition LParenCondition) { }
    public void visit(FormalParamList FormalParamList) { }
    public void visit(Expr Expr) { }
    public void visit(ConstSingleSpec ConstSingleSpec) { }
    public void visit(MethodTypeName MethodTypeName) { }
    public void visit(ActualPars ActualPars) { }
    public void visit(BoolOp BoolOp) { }
    public void visit(MethVarDeclList MethVarDeclList) { }
    public void visit(Statement Statement) { }
    public void visit(ConstFactor ConstFactor) { }
    public void visit(EqualOp EqualOp) { }
    public void visit(AddOperations AddOperations) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(VarType VarType) { }
    public void visit(MulOperations MulOperations) { }
    public void visit(FormPars FormPars) { }
    public void visit(ConstType ConstType) { }
    public void visit(NotEq NotEq) { visit(); }
    public void visit(LeEq LeEq) { visit(); }
    public void visit(Less Less) { visit(); }
    public void visit(Greater Greater) { visit(); }
    public void visit(GteEq GteEq) { visit(); }
    public void visit(EqualBool EqualBool) { visit(); }
    public void visit(DivEq DivEq) { visit(); }
    public void visit(MulEq MulEq) { visit(); }
    public void visit(SubEq SubEq) { visit(); }
    public void visit(AddEq AddEq) { visit(); }
    public void visit(EqualNode EqualNode) { visit(); }
    public void visit(Modop Modop) { visit(); }
    public void visit(Divop Divop) { visit(); }
    public void visit(Mulop Mulop) { visit(); }
    public void visit(Subop Subop) { visit(); }
    public void visit(Addop Addop) { visit(); }
    public void visit(DesignatorLbrackNode DesignatorLbrackNode) { visit(); }
    public void visit(DesignatorArrayMemberNode DesignatorArrayMemberNode) { visit(); }
    public void visit(DesignatorNode DesignatorNode) { visit(); }
    public void visit(ActualParam ActualParam) { visit(); }
    public void visit(ActualParams ActualParams) { visit(); }
    public void visit(NoActuals NoActuals) { visit(); }
    public void visit(Actuals Actuals) { visit(); }
    public void visit(NothNode NothNode) { visit(); }
    public void visit(DecNode DecNode) { visit(); }
    public void visit(IncNode IncNode) { visit(); }
    public void visit(ConstBool ConstBool) { visit(); }
    public void visit(ConstChar ConstChar) { visit(); }
    public void visit(ConstInt ConstInt) { visit(); }
    public void visit(NewArrayNode NewArrayNode) { visit(); }
    public void visit(FuncCall FuncCall) { visit(); }
    public void visit(ParenExpFactorNode ParenExpFactorNode) { visit(); }
    public void visit(Var Var) { visit(); }
    public void visit(ConstFactorNode ConstFactorNode) { visit(); }
    public void visit(TermNode TermNode) { visit(); }
    public void visit(TermMulopNode TermMulopNode) { visit(); }
    public void visit(TermExpr TermExpr) { visit(); }
    public void visit(BoolExpr BoolExpr) { visit(); }
    public void visit(EqExpr EqExpr) { visit(); }
    public void visit(OppExpr OppExpr) { visit(); }
    public void visit(NegativeExp NegativeExp) { visit(); }
    public void visit(RegularExp RegularExp) { visit(); }
    public void visit(DeclNodeStmt DeclNodeStmt) { visit(); }
    public void visit(DecrementOnlyNode DecrementOnlyNode) { visit(); }
    public void visit(IncrementOnlyNode IncrementOnlyNode) { visit(); }
    public void visit(ProcCall ProcCall) { visit(); }
    public void visit(BlockNode BlockNode) { visit(); }
    public void visit(ReturnNoExpr ReturnNoExpr) { visit(); }
    public void visit(ReturnExpr ReturnExpr) { visit(); }
    public void visit(ReadStmt ReadStmt) { visit(); }
    public void visit(PrintStmt PrintStmt) { visit(); }
    public void visit(ErrAssignment ErrAssignment) { visit(); }
    public void visit(Assignment Assignment) { visit(); }
    public void visit(NoStmt NoStmt) { visit(); }
    public void visit(Statements Statements) { visit(); }
    public void visit(FormalParamDecl FormalParamDecl) { visit(); }
    public void visit(SingleFormalParamDecl SingleFormalParamDecl) { visit(); }
    public void visit(FormalParamDecls FormalParamDecls) { visit(); }
    public void visit(NoFormParam NoFormParam) { visit(); }
    public void visit(FormParams FormParams) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(VoidNode VoidNode) { visit(); }
    public void visit(MethTypeNode MethTypeNode) { visit(); }
    public void visit(MethNoVarDecl MethNoVarDecl) { visit(); }
    public void visit(MethConstDeclNode MethConstDeclNode) { visit(); }
    public void visit(MethVarDeclarations MethVarDeclarations) { visit(); }
    public void visit(MethodTypeNameNode MethodTypeNameNode) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(NoMethodDecl NoMethodDecl) { visit(); }
    public void visit(MethodDeclarations MethodDeclarations) { visit(); }
    public void visit(ConstSingleSpecNode ConstSingleSpecNode) { visit(); }
    public void visit(MultiConstNode MultiConstNode) { visit(); }
    public void visit(SingleConstNode SingleConstNode) { visit(); }
    public void visit(ConstTypeNode ConstTypeNode) { visit(); }
    public void visit(ConstDecl ConstDecl) { visit(); }
    public void visit(ArrayDeclNodeErr ArrayDeclNodeErr) { visit(); }
    public void visit(AssNodeError AssNodeError) { visit(); }
    public void visit(ArrayDeclNode ArrayDeclNode) { visit(); }
    public void visit(AssNode AssNode) { visit(); }
    public void visit(NoAssNode NoAssNode) { visit(); }
    public void visit(SingleSpecNode SingleSpecNode) { visit(); }
    public void visit(MultiVarNode MultiVarNode) { visit(); }
    public void visit(SingleVarNode SingleVarNode) { visit(); }
    public void visit(VarTypeNode VarTypeNode) { visit(); }
    public void visit(VarDecl VarDecl) { visit(); }
    public void visit(NoVarDecl NoVarDecl) { visit(); }
    public void visit(ConstDeclNode ConstDeclNode) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(ProgName ProgName) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
